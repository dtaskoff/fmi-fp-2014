...
(define (rows mark board)
  (any id (map (lambda (row)
                 (all (lambda (x) (equal? mark x)) row))
               board)))

(define (cols mark board)
  (rows mark (transpose board)))

(define (diag1 mark board)
  (if (null? board) #t
      (and (equal? mark (caar board))
           (diag1 mark (map cdr (cdr board))))))

(define (diag2 mark board)
  (diag1 mark (reverse board)))
